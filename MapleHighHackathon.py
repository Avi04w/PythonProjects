# Avi W, Daniel T, Brandon D, Ameya S
# Smiley Smoothie
# Level 2
...
straw_sold = random.randint(375, 575)  # generates random values for sales
berry_sold = random.randint(375, 575)
mango_sold = random.randint(325, 525)
...
while True:  # will repeat code if invalid input is entered
 ...
 10. End program """)
    ...
    # if 3 is chosen, asks the user how much of a certain ingredient they want to purchase and whether or not they
    # can afford it
    elif option == '3':
        for i in ingredients:
    ...
    # if 4 is chosen, allows the user to remove a certain amount of a specific inventory item.
        ...
        while True:
            ...
            print("")
            del_option = input(
                "Which ingredient would you like to remove inventory from? (input the corresponding number "
                "represented to the left of the ingredient name. If none, type '0') ")
            if del_option == "1":
    ...
    # if 6 is chosen, allows the user to report any sales that they make. Balance will increase and inventory will
    # decrease based on how many products are sold. If they do not have the inventory to make the sale, it will state
    # that they cannot do so
        ...
        total_sales = strawberry_sales * (menu["Strawberry Smoothie"]) + mango_sales * (
            menu["Mango Smoothie"]) + berry_sales * (menu["Berry Blast Smoothie"])

        print("Your total sales are: $%.2f" % total_sales)

    # if 7 is chosen, it will display revenue and expenses. It will then show which loans/subsidy the user is
    # eligible for and allows them to choose one, affecting their balance and interest on expenses
    elif option == "7":
        ...
        print("- Owners don't need to employ anyone.")
        ...
        if len(eligible) == 0:
            ...
            time.sleep(5)
            print(
                "Which of these eligible loans would you like to choose from (Input bank name or type '0' for none)? ")
            for i in eligible:
                print("- " + i)
    ...
    elif option == "8": # if 8 is chosen, the program allows the user to pay off part of their loan based on how many
        # years it been since their acquired it.
        if chosen_loan == "":
            ...
            if chosen_loan == "TD":
                ...
                while True:
                    loan_repay = input(
                        "At the current time, your loan repayment is $%d. Would you like to pay some of it back "
                        "right now? Type '1' for YES or '0' for NO: " % amount_due)
                    if loan_repay == "1":
                ...
                while True:
                    loan_repay = input(
                        "At the current time, your loan repayment is $%.2f. Would you like to pay some of it back "
                        "right now? Type '1' for YES or '0' for NO: " % amount_due)
                    if loan_repay == "1":
                        payment = float(input("How much do you want to pay? "))
                        if balance >= payment:
